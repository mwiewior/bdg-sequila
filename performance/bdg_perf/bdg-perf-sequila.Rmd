...
for(t in all_test_df$query_id){
  query <- stringr::str_interp("SELECT test_id,cast(elapsed_time as integer) as elapsed_time  FROM ${bdg_table} WHERE query_id='${t}' AND test_id<>'master' ORDER BY time_stamp desc LIMIT 5")
